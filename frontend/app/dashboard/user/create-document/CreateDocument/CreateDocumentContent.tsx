// @ts-nocheck
  ...
  import { Download, Save, FileText, ChevronDown, ChevronRight, User, Layout, List, RotateCcw, X, Pencil, Eye } from "lucide-react";
   ...
   const [pendingSowType, setPendingSowType] = useState<'full' | 'small' | 'proposal' | null>(null);
    ...
    useEffect(() => {
      ...
      try {
        ...
        setPartnerName(customerInfo?.partnerName || "");
      } catch (error) {
    ...
    // Load draft for current OPE and merge module metadata so documentSections survives reload
    // useEffect(() => {
    //   if (!opeId) return;
    ...
    //       // If no server draft, check local pendingDraft for same ope
    //       let pending = null;
    ...
    //         // Merge modules with authoritative modules array so runtime fields (name, canEdit, description) exist
    ...
    //       // If pending local draft exists for this ope, flush it to server
    ...
    //   loadDraftForOpe();
    //   // re-run when modules change so we can merge in metadata once modules arrive
    // }, [opeId, modules]);
    ...
    const loadDraftForOpe = async () => {
      ...
      try {
        ...
        if (source && source.content && Array.isArray(source.content.documentSections)) {
          ...
          const loadedSowType =
          source.sowType === 'SMALL'    ? 'small'    :
          source.sowType === 'PROPOSAL' ? 'proposal' : 'full';
          setSowSize(loadedSowType);
          console.log("📂 Loaded draft for OPE", opeId, "| sowType =", loadedSowType);
          
          const draftVersion = source.version || 1;  // fallback if version is missing
          const newDocumentName = generateDocumentName(loadedSowType, draftVersion, "draft");
  ...
  const partnerPart = partnerName && partnerName.trim() ? ` to ${partnerName}` : "";
  const sowLabel = type === 'proposal' ? "Proposal" : "SoW";
  const forPart = ` for ${customerName}`;
  return `${idPart} - HPE Nonstop ${sowLabel}${partnerPart}${forPart}_${status}_v${versionNumber}`;
};
  ...
  setShowSowTypeWarning(true);
   ...
   if (isSaving || autoSaveInProgress) {
      ...
      try {
        ...
        const draftDocumentName = generateDocumentName(effectiveSowType, version || 1, "draft");
        ...
        const draftData = {
          ...
          customerName,
          partnerName,
          documentName: draftDocumentName,
          ...
          sowType: (() => {
          const t = explicitSowType || sowSize;
...
const confirmSowTypeChange = async () => {
  ...
  try {
    // 1. Optimistically clear the document builder UI
    setDocumentSections([]);
    setSowSize(pendingSowType);

    // 2. Persist the empty builder + new sowType to the backend
    const res = await autoSaveDraft([], pendingSowType);
    if (!res || (typeof res === "object" && res.success === false)) {
      throw new Error("Autosave failed");
    }

    // 3. Refresh available lists so all removed sections reappear in the left pane
    await refreshSourceLists();
    
    // 4. Show accurate toast message based on the new type
    const typeLabel = 
      pendingSowType === 'full' ? 'Full' : 
      pendingSowType === 'small' ? 'Short' : 'Proposal';
    showToast(`Switched to ${typeLabel} document`);
    ...
    showToast("Failed to change document type. Changes reverted.");
  } finally {
...
const handlePreview = async () => {
  ...
  setPreviewLoading(false);
    ...
    const handleGenerate = async (type: string, status: 'draft' | 'final') => {
      ...
      sowSize === 'proposal'
        ? partnerName
          ? `${idPart} - HPE Nonstop Proposal to ${partnerName} for ${customerName}_${status}_v${nextVersion}`
          : `${idPart} - HPE Nonstop Proposal for ${customerName}_${status}_v${nextVersion}`
        : partnerName
          ? `${idPart} - HPE Nonstop SoW to ${partnerName} for ${customerName}_${status}_v${nextVersion}`
          : `${idPart} - HPE Nonstop SoW for ${customerName}_${status}_v${nextVersion}`;
...
const formatDateOnly = (date = new Date()) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div>
                ...
                <div className="px-4 py-2 bg-gray-50 border rounded-lg">
                  <p className="font-medium">{customerName || "null"}</p>
                </div>
              </div>
              <div>
  ...
  <label className="inline-flex items-center gap-1.5 cursor-pointer">
    <input
      type="radio" name="sowSize" value="full"
  ...
  <label className="inline-flex items-center gap-1.5 cursor-pointer">
    <input
      type="radio" name="sowSize" value="small"
  ...
  <label className="inline-flex items-center gap-1.5 cursor-pointer">
    <input
      type="radio" name="sowSize" value="proposal"
      checked={sowSize === 'proposal'}
      onChange={() => handleSowTypeChange('proposal')}
      className="w-4 h-4 accent-blue-600"
    />
    <span className="text-blue-700 font-semibold">Proposal</span>
  </label>
</div>
              </div>
        ...
        <GenerateDocumentModal
          ...
          userEmail={userEmail}
            sowType={(() => {
        if (sowSize === 'small') return 'SMALL';
        if (sowSize === 'proposal') return 'PROPOSAL';
        return 'FULL';
        })()}
        />
