...
const handleDownloadVersion = async (versionObj: any, type: 'docx' | 'pdf') => {
  ...
  try {
    ...
    if (response && response.size > 0) {
      ...
      // Prefer the filename stored on the version record (versionData.fileName),
      // then payload.fileName, then fallback to composed name.
      const candidate = versionData.fileName || payload.fileName || `${documentName}_${versionObj.status}_v${versionObj.version}`;
  ...
  const openEmailModal = async (versionObj: any) => {
    ...
    // Force the extension to match requested fileType even if versionObj.fileName exists
    const defaultFileName = ensureExtension(versionObj.fileName || `${documentName}_${versionObj.status}_v${versionObj.version}`, fileType);
    ...
    try {
      // fetch the version from DB to build payload (same as download flow)
      const dbVersion = await fetchVersionFromDb(versionObj.version, versionObj.status);
      const versionData = dbVersion.draft || dbVersion.final || versionObj;
      const payload = buildDocumentPayload(versionData);

      // Request generated file as blob
      ...
      if (response && response.size > 0) {
        ...
        const a = document.createElement("a");
        a.href = url;
        // ensure download filename uses correct extension
        a.download = ensureExtension(defaultFileName, fileType);
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      ...
      console.warn("openEmailModal: download failed, opening mail client anyway", err);
    } finally {
      // Open mail client regardless of download success
      const mailto = `mailto:?subject=${subject}&body=${body}`;
      // clear loading action shortly after opening mail client
      try { window.location.href = mailto; } finally { setLoadingAction(null); }
    }
  };
